...
from yomitoku_client.parsers.sagemaker_parser import DocumentResult, Paragraph, Word
...
class TestYomitokuClient:
    ...
    def test_convert_to_format_with_output_path(self):
        ...
        try:
            client.convert_to_format([data], "csv", output_path=output_path)
    ...
    def test_convert_to_format_with_kwargs(self):
        ...
        result = client.convert_to_format(
            [data], "html", ignore_line_break=True, export_figure=False
        )
        assert isinstance(result, str)
        assert len(result) > 0
    ...
    def test_client_with_complex_data(self):
        ...
        paragraph1 = Paragraph(
            contents="First paragraph", box=[10, 10, 100, 30], order=1, role="paragraph"
        )
