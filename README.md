...
## スループット
以下は MLism 社内検証に基づく実測値です。帳票、手書きメモ、新聞などを含む同一の文書509ページを、東京リージョン（`ap-northeast-1`）の各構成で非同期推論・同時実行4により処理した際のスループットと、そこから試算した1ページあたりコストです。

| インスタンスタイプ         | モデル | 実測スループット（ページ/時） | 1ページあたりコスト（ライセンス込み・概算） |
| ------------------- | ---- | ------------------: | ---------------------: |
| **ml.g4dn.xlarge**  | 通常版 | 1,512               | 約 1.16 円               |
| **ml.g4dn.xlarge**  | 軽量版 | 2,952               | 約 0.60 円               |
| **ml.g5.xlarge**    | 通常版 | 3,276               | 約 0.59 円               |
| **ml.g5.xlarge**    | 軽量版 | 4,356               | 約 0.44 円               |
| **ml.g6.xlarge**    | 通常版 | 2,376               | 約 0.78 円               |
| **ml.g6.xlarge**    | 軽量版 | **4,788**           | **約 0.39 円**           |
| **ml.c7i.2xlarge**  | 軽量版 | 900                 | 約 1.87 円               |

> **Notes**
> - 軽量版はAWS Marketplace上の別プロダクトとして提供しており、`yomitoku-client sagemaker deploy` の `--product document-analyzer-lite` オプションで選択できます。なお、CPUインスタンス（c7i）は通常版のプロダクトでも常に軽量版で動作します。
...
リアルタイム処理などのユースケースでSageMakerエンドポイントを長時間 / 常時運用をご希望する場合は、プライベートオファーにて割引価格での提供も行っています。
