...
class SearchablePDFGenerator:
    ...
    def _register_font(self) -> None:
        ...
        try:
            ...
            print(
                f"Warning: Failed to register font, falling back to Helvetica: {e}")
        ...
        **kwargs,
        ...
        try:
            ...
            raise FormatConversionError(
                f"Failed to create searchable PDF: {e}")

    def _add_ocr_text(
        self, canvas_obj, ocr_result: Any, page_width: int, page_height: int
    ) -> None:
        """
        ...
        for word in ocr_result.words:
            ...
            if direction == "vertical":
                self._draw_vertical_text(
                    canvas_obj, text, x1, y1, y2, font_size, page_height
                )
            else:
                self._draw_horizontal_text(
                    canvas_obj, text, x1, y1, y2, bbox_height, font_size, page_height
                )
    ...
    def _calc_font_size(
        self, content: str, bbox_height: float, bbox_width: float
    ) -> float:
        """
    ...
    def _draw_vertical_text(
        ...
        for j, ch in enumerate(text):
            ...
            canvas_obj.translate(x1 + font_size * 0.5,
